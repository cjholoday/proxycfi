    ...
    for descr in asm_file_descrs:
        ...
        for funct in functs:
            ...
            for site in funct.sites:
                num_lines_to_write = site.line_num - asm_line_num
                write_lines(num_lines_to_write, asm_src, asm_dest)

                line_to_fix = asm_src.readline()
                asm_line_num = site.line_num + 1
                
                convert_to_cdi(site, funct, line_to_fix, asm_dest, cfg,
                        sled_id_faucet, options)
...
def convert_to_cdi(site, funct, asm_line, asm_dest, cfg, sled_id_faucet, options):
    """Converts asm_line to cdi compliant code then writes it to asm_dest"""

    if site.group == site.CALL_SITE:
        convert_call_site(site, funct, asm_line, asm_dest, sled_id_faucet, options)
    elif site.group == site.RETURN_SITE:
        convert_return_site(site, funct, asm_line, asm_dest, cfg, sled_id_faucet, options)
        ...
        eprint('warning: site has invalid type: line ' + site.line_num, 
...
def convert_call_site(site, funct, asm_line, asm_dest, sled_id_faucet, options):
    ...
    call_sled += '1:\n'
    call_sled += cdi_exit_str(sled_id_faucet(), funct.asm_filename, options.debug_mode)
    call_sled += '2:\n'
    asm_dest.write(call_sled)

        
def convert_return_site(site, funct, asm_line, asm_dest, cfg, sled_id_faucet, options):
    ...
    ret_sled += cdi_exit_str(sled_id_faucet(), funct.asm_filename, options.debug_mode)
...
def cdi_exit_str(sled_id, asm_filename, debug_mode):
    call_cdi_exit = ''
    if debug_mode:
        ...
        call_cdi_exit += '\t.string\t"' + asm_filename + ' id=' + str(sled_id) + '"\n'
