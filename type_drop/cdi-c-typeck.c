...
build_component_ref (location_t loc, tree datum, tree component)
{
  ...
  if (code == RECORD_TYPE || code == UNION_TYPE)
      ...
      do
	{
      ...
      if (ref && TREE_CODE(TREE_TYPE(ref)) == POINTER_TYPE) {
          cdi_print_fp_info(cdi_fptype_file(), TREE_TYPE(ref), loc);
      }
      return ref;
    }
  ...
  if (decl && decl != error_mark_node)
    {
      ...
      *type = TREE_TYPE (ref);
      ...
      /* CDI EDIT
       *
       * cdi_function_ahead is set at all points in the code right before
       * build_external_ref is called. Because the parser is only 
       * accessible in c-parser.c, we can only detect whether a function is 
       * ahead in c-parser.c. Hence the need for the global variable in the 
       * first place.
       *
       * Note that fun is insufficient. For example,
       *    int (*oper)(int, int) = add;
       *    (((oper)))(1, 2);
       *
       * fun is set to true iff the next token is an open parenthesis. So
       * fun will not be set true even though oper is being used as a function
       *
       * See cdi_is_function_ahead() for details on the workaround
       */
      if (type && TREE_CODE(*type) == 10) {
          cdi_print_fp_info(cdi_fptype_file(), *type, loc);
      }
    }
  else if (fun)
    {
