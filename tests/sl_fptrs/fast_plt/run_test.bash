...
cdi-gcc --cdi-make-sl=libadd.so.1.0.0 add.c
check "compilation of libadd.so failed" || exit 1

cdi-gcc --cdi-make-sl=libdo_add.so.2.0.0 --cdi-use-sl=libadd.so do_add.c
check "compilation of libdo_add.so failed" || exit 1

cdi-gcc --cdi-use-sl=libadd.so.1,libdo_add.so main.c -o out
