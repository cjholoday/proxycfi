...
# endif
	...
	if (lazy)
	{
		_dl_debug_printf ("elf_machine_rel -- Lazy\n");
...
# endif
...
#endif
		{
			...
			// _dl_debug_printf ("elf_machine_rel--1 -> %llx - %llx",(unsigned long long )(r), (unsigned long long)(end));
			unsigned int i = 0;
			for (; r < end; ++r)
			{
#if defined ELF_MACHINE_IRELATIVE && !defined RTLD_BOOTSTRAP
				...
				_dl_debug_printf ("elf_machine_rel--1 - %u\n", i++);
			...
			if (r2 != NULL){
				...
				for (; r2 <= end2; ++r2)
					if (ELFW(R_TYPE) (r2->r_info) == ELF_MACHINE_IRELATIVE)
					{
						...
						_dl_debug_printf ("elf_machine_rel--2 i = %u, j = %u\n", i, j++);
...
# endif
				{
					_dl_debug_printf ("elf_machine_rel--3\n");
...
# ifdef ELF_MACHINE_IRELATIVE
			if (r2 != NULL)
				for (; r2 <= end2; ++r2)
					if (ELFW(R_TYPE) (r2->r_info) == ELF_MACHINE_IRELATIVE){
						_dl_debug_printf ("elf_machine_rel--4\n");
