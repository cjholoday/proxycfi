...
# endif
	...
	_dl_debug_printf("****1 -> CDI Inside _dl_start_final 1**** \n");
...
dl_main (const ElfW(Phdr) *phdr,
		...
		ElfW(auxv_t) *auxv)
{
	_dl_debug_printf("****2 -> CDI Inside dl_main 2**** \n");
	const ElfW(Phdr) *ph;
	enum mode mode;
		...
		else
		{
			...
			if (GLRO(dl_lazy) >= 0 && GLRO(dl_verbose))
			{
						...
						&& rtld_multiple_ref)
				{
					...
					_dl_debug_printf ("CDI rtld 1841\n");
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX (DT_VERNEED))
	...
	else
	{
		...
		while (i-- > 0)
		{
			...
			if (l != &GL(dl_rtld_map)){
					_dl_debug_printf ("CDI rtld 2070 ********\n");
	...
	if (! prelinked && rtld_multiple_ref)
	{
		...
		/* Mark the link map as not yet relocated again.  */
		GL(dl_rtld_map).l_relocated = 0;
			_dl_debug_printf ("CDI rtld 2129\n");
...
{
	_dl_debug_printf("****3 -> CDI Inside process_envvars 3****\n");
	char **runp = _environ;
	char *envline;
