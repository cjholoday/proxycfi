...
/* CDI Modifications to plt entries */
/* Original*/
...
// static const bfd_byte elf_x86_64_plt_entry[PLT_ENTRY_SIZE] =
// {
// 	0xff, 0x15,  /* callq *name@GOTPC(%rip) */
// 	0, 0, 0, 0,  /* replaced with offset to this symbol in .got.  */
// 	0xff, 0x25,   /* jmp relative */
// 	0, 0, 0, 0,  /*replaced with the start address of the RLT for this function  */
// 	0x90, 0x90, 0x90, 0x90  /* replaced with index into relocation table.  */
// };
// static const bfd_byte elf_x86_64_plt_entry[PLT_ENTRY_SIZE] =
// {
// 	0xe8,			/* callq relative distance(%rip) */
// 	0, 0, 0, 0, 	/* replaced with relative distance to the actual lib_function */
// 	0xe9,			/* callq replative distance(%rip)  */
// 	0, 0, 0, 0,		/* replaced with the relative distance to the RLT section of the lib_function */
// 	0x90, 0x90, 0x90, 0x90, 0x90, 0x90   /* NOPs  */
// };
/* CDI Modification based on:
   http://workblog.pilin.name/2016/10/full-address-space-jumps-for-hooks_26.html
*/
static const bfd_byte elf_x86_64_plt_entry[PLT_ENTRY_SIZE] =
{
	0xff, 0x15,  /* callq *name@GOTPC(%rip) */
...
/* CDI Modifications in this functions */
	static bfd_boolean
		...
		Elf_Internal_Sym *sym ATTRIBUTE_UNUSED)
{
	...
	if (h->plt.offset != (bfd_vma) -1)
	{
		...
		/* CDI --> .plt section is filled in here */
		...
		/* CDI Modification--> this instruction replaces the address of the first instruction in the plt entry aka call */
		/* This should be removed ==> as this is supposed to be done by the loader */
		/* If the call instruction is changed from 0xFF25 to 0xE8 subtract 1 to get the actual address to change*/
		...
		if (plt == htab->elf.splt){
			...
			/* CDI Modifications  */
			/* This instruction changes the relative address in the second instruction of the plt entry*/
			/* Replace it with the relative distance from the corresponding rlt_entry of this plt_entry*/
			/* rlt_index + start_of.rlt Section - address of next instruction */
			/* address of this instuction ==> plt->contents + h->plt.offset+ abed->plt_reloc_offset + 5 */
			/* 5 => size of this instruction in bytes */
			bfd_put_32 (output_bfd, (bfd_vma)(plt_index + 0x08010000),
					plt->contents + h->plt.offset+ abed->plt_reloc_offset + 1);
			...
			/** CDI Modification --> Avoids replacing the address in the 3rd instruction in every plt entry ==> Not executed*/
			/* already replace with 6 NOPs */
			/*    bfd_put_32 (output_bfd, - plt0_offset, plt->contents + h->plt.offset + plt_plt_offset);*/
		}
...
bad_return:
	...
	for (i = 0; i < count; i++, p++)
	{
		...
		/* CDI  Modified -- May be required to disable the next if */
