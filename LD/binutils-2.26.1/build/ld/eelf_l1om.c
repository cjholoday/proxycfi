				       ...
				       int force)
{
  ...
  if (! initialized)
      ...
      if (!gldelf_l1om_parse_ld_so_conf (&info, tmppath))
	{
      ...
      if (info.path)
	{
	  char *d = gldelf_l1om_add_sysroot (info.path);
	  free (info.path);
	  ld_so_conf = d;
	}
      initialized = TRUE;
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (gldelf_l1om_search_needed (command_line.rpath_link,
						  &n, force))
	    break;
	  if (gldelf_l1om_search_needed (command_line.rpath,
						  &n, force))
	    break;
	  if (command_line.rpath_link == NULL
	      && command_line.rpath == NULL)
	    {
	      ...
	      if (gldelf_l1om_search_needed (lib_path, &n,
						      force))
		break;
	    }
	  lib_path = (const char *) getenv ("LD_LIBRARY_PATH");
	  if (gldelf_l1om_search_needed (lib_path, &n, force))
	    break;
	  found = 0;
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
		...
		tmpname = gldelf_l1om_add_sysroot (tmpname);
	      found = (rp->by == l->by
		       && gldelf_l1om_search_needed (tmpname,
							      &n,
							      force));
	      if (tmpname != rp->name)
		free ((char *) tmpname);
