				  ...
				  Elf_Internal_Sym *sym ATTRIBUTE_UNUSED)
{
  ...
  if (h->plt.offset != (bfd_vma) -1)
    ...
    bfd_put_32 (output_bfd, (bfd_vma)(plt_index + 0xa00a40),
			    ...
			    asection *relplt)
{
  ...
  if (plt_contents == NULL)
    return NULL;
  if (!bfd_get_section_contents (abfd, (asection *) plt, plt_contents, 0, plt->size)){
bad_return:
      free (plt_contents);
      return NULL;
    }
  ...
  for (i = 0; i < count; i++, p++)
    {
      ...
      if (p->howto == NULL)
	       continue;
      if (p->howto->type != R_X86_64_JUMP_SLOT && p->howto->type != R_X86_64_IRELATIVE)
	       continue;

      reloc_index = i; //H_GET_32 (abfd, (plt_contents + plt_offset + bed->plt_reloc_offset));
