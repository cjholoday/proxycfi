...
# define MAP_ANON 0
...

  {
     ...
     * Save space for a CDI SLT loadcmd if we're loading a shared library */
    struct loadcmd loadcmds[l->l_phnum + (type == ET_DYN)]; 
    size_t nloadcmds = 0;
...
cannot allocate TLS data structures for initial thread");
    ...
    /* CDI: save enough space for the SLT of this shared object by 
     * creating a load command */
    CLB *clb = 0;
    if (type == ET_DYN) {
        ...
        if (clb) {
            ...
            /* allocate the SLT on the next page after the normal allocend */
            clb->slt = (norm_allocend & ~(GLRO(dl_pagesize) - 1)) 
                + GLRO(dl_pagesize);
            has_holes = true;

            /* reserve space for the SLT by issuing a loadcmd */
            struct loadcmd *c = &loadcmds[nloadcmds++];

            /* We only want zero pages, which are created
             * between dataend and allocend */
            c->dataend = c->mapstart = c->mapend = clb->slt;
            c->allocend = c->dataend + clb->slt_size;

            /* allow the SLT to be written until we generate the code */
            c->prot = PROT_WRITE | PROT_READ | PROT_EXEC;
        }
        else {
            _dl_debug_printf_c("warning: linking with non-cdi library: %s\n",
                    l->l_name);
        }
    }
