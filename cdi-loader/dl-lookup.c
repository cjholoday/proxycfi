...
ElfW(Addr) _cdi_lookup(const char *sym_str, struct link_map *l) {
    ...
    /*
    do_lookup_x
    lookup_t lookup = _dl_lookup_symbol_x (sym_str, l, &ref, scope_ptrs, 
    ...
    const uint_fast32_t new_hash = dl_new_hash (sym_str);
    unsigned long int old_hash = 0xffffffff;

    struct sym_val found_sym; 
    int found = do_lookup_x(sym_str, new_hash, &old_hash, 0, &found_sym, 
                            &scope, 0, 0, DL_LOOKUP_RETURN_NEWEST, 0, 0, l);

    return found ? (ElfW(Addr)) l->l_addr + found_sym.s->st_value : 0;
}
